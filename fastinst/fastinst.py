...
class FastInst(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        obj_key_weight = cfg.MODEL.FASTINST.OBJ_KEYP_WEIGHT
        hand_key_weight = cfg.MODEL.FASTINST.HAND_KEYP_WEIGHT
        location_weight = cfg.MODEL.FASTINST.LOCATION_WEIGHT
        proposal_weight = cfg.MODEL.FASTINST.PROPOSAL_WEIGHT

        delta_hand_weight = cfg.MODEL.FASTINST.DELTA_HAND_WEIGHT
        delta_obj_weight = cfg.MODEL.FASTINST.DELTA_OBJ_WEIGHT
        ...
        weight_dict = {"loss_ce": class_weight,
                       ...
                       "loss_hand_keypoint": hand_key_weight,
                       "loss_hand_2dkeypoint": hand_key_weight,
                       "loss_obj_keypoint": obj_key_weight,
                       "loss_delta_obj": delta_obj_weight,
                        "loss_delta_hand": delta_hand_weight,
                       "loss_contact_map": 10.0
                       }
        ...
        losses = ['labels', 'cardinality', 'hand_keypoint', "hand_2dkeypoint", 'obj_keypoint', 'delta_hand', 'delta_obj']
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            images, targets = batched_inputs[0].tensors, batched_inputs[1]
        else:
            if type(batched_inputs) is list:
                images = batched_inputs[0].tensors
            else:
                # images = batched_inputs["images"].tensors
                images = batched_inputs.tensors
