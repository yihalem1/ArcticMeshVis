...
def pixel2cam(pixel_coord, f, c, T_=None):
    ...
    if T_ is not None:  # MANO space와 scale과 wrist를 맞추고자
        ...
        # else:
        #     T_ = torch.stack(T_)
        ratio = torch.linalg.norm(T_[:, 9] - T_[:, 0], dim=-1) / torch.linalg.norm(
            cam_coord[:, :, 9] - cam_coord[:, :, 0], dim=-1)
...
def test_pose(model, criterion, data_loader, device, cfg, vis=False, save_pickle=False):
    ...
    for _ in pbar:
        ...
        with torch.no_grad():
            ...
            ####################################################
            # model output
            out_logits, pred_keypoints, pred_obj_keypoints = outputs['pred_logits'], outputs['pred_keypoints'], outputs[
                'pred_obj_keypoints']
            ...
            obj_kp = torch.gather(pred_obj_keypoints, 1, obj_idx.unsqueeze(1).unsqueeze(1).repeat(1, 1, 63)).reshape(B,
                                                                                                                     21,
                                                                                                                     3)
            ...
            hand_kp[..., :2] *= target_sizes.unsqueeze(1).unsqueeze(1);
            hand_kp[..., 2] *= 1000
            ...
            for i, batch in enumerate(gt_labels):
                ...
                for k, label in enumerate(batch):
                    if dataset == 'H2O':
                        if label == cfg.hand_idx[0]:
                            j = 0
                        elif label == cfg.hand_idx[1]:
                            j = 1
                        else:
                            j = 2
                    else:
                    ...
                    gt_joint_cam = pixel2cam(gt_scaled_keypoints, (cam_fx.item(), cam_fy.item()),
                                             (cam_cx.item(), cam_cy.item()))
        ...
        if save_pickle:
            assert data_loader.batch_size == 1
            save_path = os.path.join(f'./pickle/{dataset}/{data_loader.dataset.mode}', filename)
            obj_label = labels[:, -1, :cfg.hand_idx[0]].argmax(-1)
            GT_3D_bbox = GT_3D_bbox_dict[obj_label.item()][None]
            pred_obj_cam = pixel2cam(obj_kp, (cam_fx.item(), cam_fy.item()),
                                     (cam_cx.item(), cam_cy.item())).detach().cpu().numpy()
            ...
            T_ = torch.stack([T_keypoints_left, T_keypoints_right]) if hand_kp.shape[1] == 2 else T_keypoints_right[
                None]
            ...
            all_cam = pixel2cam(key_points, (cam_fx.item(), cam_fy.item()), (cam_cx.item(), cam_cy.item())).reshape(1,
                                                                                                                    -1)
            obj_6D = torch.cat([R.reshape(-1, 9), t], dim=-1)
            label_prob = labels[:, -1]

            data = {'uvd': all_uvd.detach().cpu().numpy(), 'cam': all_cam.detach().cpu().numpy(),
                    '6D': obj_6D.detach().cpu().numpy(), 'label': label_prob.detach().cpu().numpy(),
                    'mano': pose_params.detach().cpu().numpy()}
        ...
        if vis:
            ...
            hand_v_color = [
                (cmap(hand_cmap_affordance[0, idx].detach().cpu().numpy())[:, 0, :-1] * 255).astype(np.int64) for idx in
                range(hand_verts.shape[1])]
...
def train_contact(temporal_model, mano_left, mano_right, GT_3D_bbox_dict, GT_obj_vertices_dict,
    ...
    for i, _ in enumerate(pbar):
        ...
        R = obj_6D[:, :9].reshape(-1, 3, 3)
        t = obj_6D[:, 9:]
        obj_vertices = torch.tensor(GT_obj_vertices_dict[obj_label], dtype=torch.float32)[None].repeat(R.shape[0], 1,
                                                                                                       1).cuda()
        ...
        mano_results = [mano_layer(mano_param[:, 48 * i:48 * (i + 1)], opt_tensor_shape) for i, mano_layer in
                        enumerate(MANO_LAYER)]
        hand_verts = torch.stack([m[0] for m in mano_results], dim=1)
        j3d_recon = torch.stack([m[1] for m in mano_results], dim=1)
        hand_verts = hand_verts - j3d_recon[:, :, :1] + hand_cam[:, :, :1]
        ...
        obj_nn_dist_affordance = get_NN(obj_vertices.to(torch.float32),
                                        hand_verts.reshape(num_frame, -1, 3).to(torch.float32))
        hand_nn_dist_affordance = torch.stack(
            [get_NN(hand_verts[:, idx].to(torch.float32), obj_vertices.to(torch.float32)) for idx in
             range(hand_verts.shape[1])], dim=1)
        obj_cmap_affordance = get_pseudo_cmap(obj_nn_dist_affordance)
        ...
        outputs = temporal_model(hand_info.detach(), obj_info.detach(), label[:, 1:-2].detach())
...
def test_contact(temporal_model, mano_left, mano_right, GT_3D_bbox_dict, GT_obj_vertices_dict,
                 criterion: torch.nn.Module,
                 data_loader: Iterable, optimizer: torch.optim.Optimizer,
